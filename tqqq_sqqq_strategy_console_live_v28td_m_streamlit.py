...
def fetch_historical_data(end_date, TICKER, LEVERAGED_TICKER, INVERSE_TICKER):
    ...
    try:
        # Download data starting from inception to end_date
        ...
        for t in tickers:
            ...
            if t == TICKER:
                df['high'] = all_data['High'][t]
                df['low'] = all_data['Low'][t]
        ...
        return df.dropna(subset=['close', 'high', 'low', 'Yield_Spread'])
...
def generate_signals(df, LEVERAGED_TICKER, adx_threshold, di_spread_pct, steepening_threshold, macro_buffer):
    ...
    for i in range(len(df)):
        ...
        if macro_lockout and row['Spread_Slope'] <= (steepening_threshold - macro_buffer):
            macro_lockout = False
...
def plot_pro_chart(sig_df, history_df, TICKER, start_date):
    ...
    for i, r in history_df.iterrows():
        ...
        if 'BUY' in r['Action']:
            buy = {'d': d, 'p': sig_df.loc[d, 'close'], 'v': r['Portfolio Value']}
        elif 'SELL' in r['Action'] and buy:
            win = r['Portfolio Value'] >= buy['v']
            segments.extend([
                {'t': i, 'Date': buy['d'], 'Price': buy['p'], 'Win': win}, 
                {'t': i, 'Date': d, 'Price': sig_df.loc[d, 'close'], 'Win': win}
            ])
            buy = None
    
    trade_layer = alt.Chart(pd.DataFrame(segments)).mark_line(size=4).encode(
        x='Date:T', y='Price:Q', detail='t:N', 
        color=alt.condition(alt.datum.Win, alt.value('green'), alt.value('red'))
    )
...
def main():
    ...
    with st.sidebar:
        ...
        start_dt = st.date_input("Start Date", date(2025, 1, 2))
        adx_t = st.number_input("ADX", 0.0, 30.0, 12.0)
        di_s = st.number_input("DI Spread", -1.0, 1.0, -0.8)
        st_t = st.number_input("Macro Threshold", 0.0, 1.0, 0.16)
    ...
    # SAFE SLICING: Use .asof or find first valid index
    actual_start = sig_df.index[sig_df.index.searchsorted(pd.Timestamp(start_dt))]
    sim_df = sig_df.loc[actual_start:].copy()
    ...
    c4.metric("Strategy Return", f"{((float(val)/10000)-1)*100:+.1f}%")

    st.markdown("## 📊 Performance vs QQQ")
    b1, b2 = st.columns(2)
    b1.metric("Strategy Final", f"${float(val):,.2f}")
    
    # SAFE BENCHMARK: Get the first available price on or after start date
    q_start_price = df[f'{TICKER}_close'].asof(pd.Timestamp(start_dt))
    q_ret = float(df[f'{TICKER}_close'].iloc[-1]) / float(q_start_price)
    b2.metric("B&H QQQ", f"${float(INITIAL_INVESTMENT) * q_ret:,.2f}")

    st.altair_chart(plot_pro_chart(sig_df, hist_df, TICKER, actual_start), use_container_width=True)
